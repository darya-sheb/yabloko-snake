...
static void handle_syscall(registers_t* r) {
    switch (r->eax) {
        ...
        case SYS_puts:
            ...
            break;
        case SYS_get_event: {
            struct kbd_event *user_ev = (struct kbd_event*)r->ebx;
            struct kbd_event ev;
            kbd_get_event(&ev);
            if (ev.scancode == 0) {
                r->eax = -1;
            } else {
                user_ev->scancode = ev.scancode;
                user_ev->pressed = ev.pressed;
                r->eax = 0;
            }
            break;
        }
        case SYS_get_key_state: {
            uint8_t scancode = (uint8_t)r->ebx;
            r->eax = kbd_get_key_state(scancode);
            break;
        }
        case SYS_get_ticks: {
            r->eax = ticks;
        ...
        case SYS_setgraphics: {
            ...
            vgaMode13();
            uintptr_t pa = 0xA0000;
            size_t size = 65536;
            if (map_phys_range(vm.user_task->pgdir, addr, pa, size, PTE_W | PTE_U) != 0) {
                r->eax = -1;
                break;
            }
            r->eax = addr;
