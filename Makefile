...
OBJECTS = ./kernel/kstart.o ./kernel.o ./console.o ./drivers/vga.o ./drivers/uart.o ./drivers/keyboard.o \
	...
	./fs/fs.o ./drivers/ata.o ./lib/string.o ./proc.o ./drivers/pit.o ./kernel/vm.o ./drivers/speaker.o

run: image.bin
	qemu-system-i386 -hda $< -serial mon:stdio -vga std -qmp unix:qemu-monitor-socket,server,nowait

run-nox: image.bin
	qemu-system-i386 -nographic -hda $< -serial mon:stdio -qmp unix:qemu-monitor-socket,server,nowait
...
ejudge.sh: image.bin
	...
	gzip <$^ | base64 >>$@
	echo >>$@ "===EOF"
	echo >>$@ "exec qemu-system-i386 -nographic -hda image.bin -serial mon:stdio"
	chmod +x $@
...
debug-boot-nox: image.bin mbr.elf
	qemu-system-i386 -nographic -hda $< -s -S &
	$(GDB) mbr.elf \
...
user/snake: user/snake_src/main.o user/snake_src/game.o user/snake_src/snake_body.o user/snake_src/world.o user/snake_src/render.o user/snake_src/ai.o user/crt.o
	$(LD) $(LDFLAGS) -o $@ -Ttext 0x401000 $^


user/snake_src/%.o: user/snake_src/%.cpp
	$(CXX) $(CXXFLAGS) -c $< -o $@
...
bootmain.o: bootmain.c
	$(CC) $(CFLAGS) -Os -fno-unwind-tables -fno-asynchronous-unwind-tables -g0 -c $< -o $@
