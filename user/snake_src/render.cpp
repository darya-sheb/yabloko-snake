...
void draw_text(int x, int y, const char* str, uint8_t color) {
    ...
    for (const char* p = str; *p; p++) {
        if (*p == '\n') {
            ...
            draw_char(cx, cy, *p, color);
            cx += 8;
        }
    }
}

void draw_cell(int cx, int cy, uint8_t color) {
    int x = cx * CELL_SIZE;
    int y = cy * CELL_SIZE;
...
void draw_game(const Snake& snake, const World& world, int score) {
    ...
    for (int i = 0; i < OBSTACLE_COUNT; i++) {
        Point p = world.getObstacle(i);
        draw_cell(p.x, p.y, 0x08);
    }
    for (int i = 0; i < world.appleCount(); i++) {
        Point p = world.getApple(i);
        draw_cell(p.x, p.y, 0x0C);
    }
    for (int i = 0; i < snake.length(); i++) {
        Point p = snake.segment(i);
        uint8_t col = (i == 0) ? 0x0A : 0x02;
        draw_cell(p.x, p.y, col);
    }
    char buf[32];
    draw_text(4, 4, "Score: ", 0x0F);
    int n = score;
    char* p = buf + 31;
    *p = 0;
    do {
